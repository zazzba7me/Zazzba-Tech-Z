...
export const Hero: React.FC = () => {
  ...
  // Combine user config image with defaults for a carousel effect
  const slides = [bannerConfig.image, ...DEFAULT_SLIDES.filter(s => s !== bannerConfig.image)].slice(0, 3);
    ...
    >
      ...
      <div className="relative z-10 p-6 sm:p-12 max-w-2xl w-full pb-16 sm:pb-12 animate-fade-up">
        ...
        <h1 
            ...
            dangerouslySetInnerHTML={{ __html: bannerConfig.title }}
        >
        </h1>
        ...
        <div className="flex gap-4">
            ...
            >
            {bannerConfig.buttonText} <ArrowRight className="w-4 h-4 group-hover/btn:translate-x-1 transition-transform" />
            </button>
        </div>
      </div>
      ...
      <div className="absolute bottom-6 left-6 right-6 flex justify-between items-end sm:justify-start sm:left-12 sm:right-auto sm:gap-4 z-20">
          ...
          <div className="flex gap-2 mb-1">
            {slides.map((_, index) => (
                ...
                >
                    <div 
                        className={`absolute top-0 left-0 h-full bg-white transition-all duration-300 ${index === currentSlide ? 'w-full' : 'w-0'}`}
                    ></div>
                </button>
            ))}
          </div>
      ...
      <button 
        onClick={prevSlide}
        className="absolute left-4 top-1/2 -translate-y-1/2 p-3 rounded-full bg-white/10 backdrop-blur-md text-white border border-white/10 hover:bg-white hover:text-gray-900 transition-all opacity-0 group-hover:opacity-100 hidden sm:flex"
      >
        <ChevronLeft className="w-6 h-6" />
      </button>
      <button 
        onClick={nextSlide}
        className="absolute right-4 top-1/2 -translate-y-1/2 p-3 rounded-full bg-white/10 backdrop-blur-md text-white border border-white/10 hover:bg-white hover:text-gray-900 transition-all opacity-0 group-hover:opacity-100 hidden sm:flex"
      >
